...
const { sendEmployeeEmail } = require("../../utils/mailer");
const { sendRestaurantEmail } = require("../../utils/sendRestaurantEmail");
const { sendResetPasswordEmail } = require("../../utils/sendResetPasswordEmail");
const { sendResetPasswordEmployee } = require("../../utils/sendResetPasswordEmployee");
...
exports.registerRestaurant = async (data) => {
   let { email, phone, password, restaurant_name } = data;
  ...
  await model.createRestaurant({
    ...
    await sendRestaurantEmail(email, restaurant_name);
...
exports.loginRestaurant = async (email, password) => {
  ...
  // Compare password
  const isMatch = await comparePassword(password, restaurant.password_hash);
  if (!isMatch) {
    throw new Error("INVALID_LOGIN");
  }

  // ✅ Correct payload for token
  const token = generateToken({
    restaurant_id: restaurant.restaurant_id, // must be restaurant_id
    role: "ADMIN", // must match generateToken admin branch
...
const RESET_PASSWORD_SECRET = process.env.JWT_SECRET || "supersecret";

// Token expires in 1 hour
const RESET_PASSWORD_EXPIRES = "1h";

exports.forgotPassword = async (email) => {
  // Check if restaurant exists
  const restaurant = await model.findByEmail(email);
  if (!restaurant.recordset.length) {
    const err = new Error("Email not found");
    err.code = "EMAIL_NOT_FOUND";
    throw err;
  }

  const { restaurant_name } = restaurant.recordset[0];

  // Generate JWT token
  const token =jwt.sign({ email }, RESET_PASSWORD_SECRET, {
...
exports.resetPassword = async (token, newPassword) => {
  ...
  // Update in DB
  await model.updatePasswordByEmail(email, password_hash);
};
...
if (
  restaurantResult.recordset.length > 0 &&
  restaurantResult.recordset[0].restaurant_name
) {
  restaurantName = restaurantResult.recordset[0].restaurant_name;
}
   /* ================= SEND EMAIL ================= */
sendEmployeeEmail(email, password, restaurantName)
...
exports.loginUserService = async (email, password) => {
  ...
  // Fetch employee by email (no restaurantId in input)
  ...
  // Generate JWT token with restaurant_id included
  const token = generateToken({
  ...
  return {
    ...
    user: {
      ...
      role: user.role,
      restaurant_id: user.restaurant_id, // optional for frontend use
    },
  };
};
...
const RESET_PASSWORD_SEC = process.env.RESET_PASSWORD_SECRET || "supersecretkey";
const RESET_PASSWORD_EXP = "1h"; // 1 hour
...
exports.forgotPasswordService = async (email) => {
  ...
  // Generate JWT token
  const token = jwt.sign({ userId: user.user_id }, RESET_PASSWORD_SEC, {
...
exports.resetPasswordService = async (token, newPassword) => {
  ...
  let decoded;
  try {
    decoded = jwt.verify(token, RESET_PASSWORD_SEC);
  } catch (err) {
...
exports.getDashboardByRestaurant = async (restaurantId) => {
  const pool = await db.poolPromise;
  ...
  .input("restaurantId", sql.Int, restaurantId)
  .query(`
    /* ================= REVENUE CHART LAST 7 DAYS FOR A SPECIFIC RESTAURANT ================= */
    WITH Last7Days AS (
    ...
    LEFT JOIN orders.Orders o
      ON o.restaurant_id = @restaurantId
    LEFT JOIN billing.Bills b
      ON b.order_id = o.order_id
      AND CAST(b.bill_date AS DATE) = l.dt
  ...
  return {
    ...
    topItems: popular.recordset, // same as popularDishes
...
exports.createOrderService = async (restaurant_id, user_id, orderData) => {
  ...
  if (!user_id) throw new Error("Employee ID (user_id) is required");
  ...
  try {
    ...
    if (existingCustomer.recordset.length > 0) {
      ...
      const newCustomer = await insertCustomerRequest
        .input("restaurant_id_new", sql.Int, restaurant_id)
        .input("customer_name", sql.VarChar(100), orderData.customer.customer_name || "Guest")
        .input("phone_new", sql.VarChar(20), orderData.customer.phone)
        .query(`
          INSERT INTO master.Customers
          (restaurant_id, customer_name, phone)
    ...
    const orderResult = await orderRequest
      ...
      .input("user_id", sql.Int, user_id)
      .input("customer_id", sql.Int, customer_id)
      .input("order_status", sql.VarChar(20), "CREATED")
      .query(`
        INSERT INTO orders.Orders
        (restaurant_id, table_id, user_id, customer_id, order_status)
        OUTPUT INSERTED.order_id
    ...
    for (let item of orderData.items) {
      if (!item.item_id || !item.quantity || !item.price) {
        throw new Error("Each order item must have item_id, quantity, and price");
      }
    ...
    /* ===============================
       4️⃣ INSERT PAYMENT RECORD
    =============================== */
    const paymentMethodRaw = (orderData.payment_method || "CASH").toString().trim().toUpperCase();
    const paymentMethod = paymentMethodRaw === "CASH" ? "CASH" : "ONLINE";
    ...
    /* ===============================
       6️⃣ INSERT INTO KITCHEN TABLE
    =============================== */
    const kitchenRequest = new sql.Request(transaction);
    await kitchenRequest
      .input("order_id", sql.Int, order_id)
      .input("kitchen_status", sql.VarChar(20), "PENDING")
      .query(`
        INSERT INTO kitchen.KitchenOrders
        (order_id, kitchen_status)
        VALUES (@order_id, @kitchen_status)
