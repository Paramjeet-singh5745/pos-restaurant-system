...
exports.login = async (req, res) => {
  try {
    ...
    return res.status(200).json({
      ...
      token,
      restaurant: restaurant.restaurant_id, // only restaurant ID
    });
  } catch (err) {
...
exports.loginEmployee = async (req, res) => {
  try {
    ...
    const { token, user } = await service.loginUserService(email, password);

    res.json({
      success: true,
      message: "Login successful",
      token,
      user, // includes user_id, role, restaurant_id
    });
  } catch (err) {
...
exports.forgotPasswordEmployee = async (req, res) => {
  try {
    const { email } = req.body;
...
exports.resetPasswordEmployee = async (req, res) => {
  try {
    ...
    const result = await service.resetPasswordService(token, newPassword);
...
exports.dashboard = async (req, res) => {
  try {
    ...
    const data = await service.getDashboardByRestaurant(restaurantId);
...
exports.createOrder = async (req, res) => {
  try {
    ...
    // Try to get the employee ID from possible fields
    const user_id = req.user.user_id; 
    if (!user_id) {
      return res.status(400).json({
        success: false,
        message: "Logged in employee ID not found",
      });
    }
