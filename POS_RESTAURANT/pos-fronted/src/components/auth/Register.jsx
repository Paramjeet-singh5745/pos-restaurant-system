      ...
      <section className="min-h-screen bg-[#0f0f0f] flex items-center justify-center px-4">
        <div className="w-full max-w-md sm:max-w-lg bg-[#121212] p-6 sm:p-8 rounded-2xl shadow-xl">
          ...
          <form onSubmit={submit}>
            <Input label="Employee Name" name="name" value={formData.name} onChange={handleChange} />
            <Input label="Employee Email" name="email" type="email" value={formData.email} onChange={handleChange} />
            <Input label="Employee Phone" name="phone" value={formData.phone} onChange={handleChange} />
            ...
            {/* SUBMIT */}
            <button
              disabled={!isPasswordValid || formData.phone.length !== 10 || loading}
...
const Input = ({ label, ...props }) => (
  <div className="mt-4">
    ...
    <div className="rounded-lg px-4 py-3 bg-[#1f1f1f] focus-within:ring-2 focus-within:ring-yellow-400">
      <input {...props} className="bg-transparent w-full text-white outline-none" />
    </div>
  </div>
);

const Rule = ({ ok, text }) => (
  <p className={`flex items-center gap-2 ${ok ? "text-green-400" : "text-red-400"}`}>
    {ok ? "✔" : "✖"} {text}
  </p>
);
