...
const BottomNav = ({ setCustomer }) => {
  ...
  // ✅ Detect login type (Employee OR Admin)
  useEffect(() => {
    ...
    if (employeeId) {
      // Employee is logged in → use employee role
      setRole(employeeRole?.toUpperCase() || "");
  ...
  return (
    <>
      ...
      <div className="sticky bottom-0 h-20 bg-[#1f1f1f] border-t border-[#333] flex justify-around items-center z-50">
        ...
        {/* Floating Button (only WAITER on menu page) */}
      ...
      {/* Customer Modal */}
      <Model isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="Customer Details">
